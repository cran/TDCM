...
summary.option3 <- function(model, num.atts, num.time.points, attribute.names) {
  ...
  for (t in 2:num.time.points) {
    for (j in 1:num.atts) {
      ...
      if (length(attribute.names) == A) {
        ...
        temp.name.effect <- paste("Time ", (t - 1), " to Time ", t, sep = "")
  ...
  } #end time point comparison loop  (successive)
  ...
  newlist3 <- list("trans" = trans, "growth" = growth,
