...
#' @keywords internal
#' @noRd
mg.summary2 <- function(model, num.atts, num.groups, num.time.points, attribute.names, group.names) {
  transition.option <- 2
  A <- num.atts
  ...
  for (g in 1:num.groups) {
    ...
    for (t in 2:num.time.points) { #open time points loop
      ...
      for (j in 1:num.atts) { #open attributes loop
      ...
      temp.name2 <- paste("Time 1 to Time ", t, sep = "") # Creates temporary name for each matrix
  ...
  newlist2 <- list("trans" = all.trans, "growth" = all.growth,
